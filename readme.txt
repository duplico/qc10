Hardware
========
* CC BY 3.0 <http://creativecommons.org/licenses/by/3.0/>

** Based in part on JeeNode, (c) Jean-Claude Wippler, et al. <http://jeelabs.net/users/1>

** Otherwise, (c) Evan G Mackay

Software
========
* TLC5940 library: Simplified BSD License <http://opensource.org/licenses/BSD-2-Clause>

** Matthew T. Pandina <https://sites.google.com/site/artcfox/demystifying-the-tlc5940>

* All others MIT License <http://opensource.org/licenses/MIT>

** JeeLib & certain example interface code (c) Jean-Claude Wippler, et al. <http://jeelabs.net/users/1>

** Otherwise, (c) George Louthan <http://georgerloutha.nthefourth.com>